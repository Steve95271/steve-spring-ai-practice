...
public class WebSearchDocumentRetriever implements DocumentRetriever {
    ...
    public List<Document> retrieve(Query query) {
        ...
        Assert.hasText(query.text(), "query.text() cannot be empty");

        TavilyResponsePayload tavilyResponse = restClient
                .post()
                .body(new TavilyRequestPayload(
                        query.text(),
                        "advanced", // Tell tavily to do advanced search
                        resultLimit
                ))
                .retrieve()
        ...
        List<Document> docs = new ArrayList<>(tavilyResponse.results().size());
        for (TavilyResponsePayload.Hit hit : tavilyResponse.results()) {
